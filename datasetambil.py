...
kategori = kategori_map.get(pilihan, "normal")

# Buat folder jika belum ada
if not os.path.exists(kategori):
    os.makedirs(kategori)

# Cari nomor urut otomatis
def get_next_index(folder, prefix, ext):
    idx = 1
    while os.path.exists(os.path.join(folder, f"{prefix}{idx}.{ext}")):
        idx += 1
    return idx

csv_index = get_next_index(kategori, kategori, "csv")
png_index = get_next_index(kategori, kategori, "png")

csv_file = os.path.join(kategori, f"{kategori}{csv_index}.csv")
png_file = os.path.join(kategori, f"{kategori}{png_index}.png")

print(f"📂 Dataset akan disimpan sebagai: {csv_file}, {png_file}")
